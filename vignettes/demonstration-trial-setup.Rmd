---
title: "Demonstration Clinical Trial: Quick Start Guide"
subtitle: "Active vs. Placebo Memory Enhancement Study with 20 Subjects"
author: "ZZedc Team"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Demonstration Clinical Trial Setup}
...
This demonstration trial is perfect for:
...
Before setting up the demonstration trial, you need to install ZZedc. If you have not
already done so, follow the installation instructions in the
...
```bash
# 1. Clone repository
mkdir ~/zzedc-project && cd ~/zzedc-project
...
If you want to skip the detailed explanations and set up the demonstration trial
immediately, use the automated setup script:

```bash
# Navigate to the zzedc directory
cd ~/zzedc-project/zzedc

# Run the complete setup script (installs dependencies, packages, and demonstration trial)
Rscript setup_demonstration_trial.R
```

This single script will:

1. Install all R dependencies
2. Install zzedc package
3. Create the demonstration trial database with 20 subjects
4. Add test users
5. Verify the setup

After the script completes, launch ZZedc:
...
```bash
Rscript vignettes/demonstration-trial/scripts/01-setup_demonstration_trial.R
Rscript vignettes/demonstration-trial/scripts/02-add_users.R
Rscript vignettes/demonstration-trial/scripts/03-verify_demonstration_trial.R
```
...
The rest of this guide explains the trial structure and setup process in
detail.
...
The demonstration trial uses CSV files to define forms and data. All templates are included in this vignette package.
...
**File Locations**: All files are in `vignettes/demonstration-trial/csv_templates/`

---

## Part 4: Choose Your Database Backend

ZZedc supports four database backends. Choose based on your deployment needs:

| Backend | Best For | Setup Complexity |
|---------|----------|------------------|
| SQLite | Local dev, single-user, getting started | None |
| DuckDB | Analytics-heavy studies, Parquet export | None |
| PostgreSQL | Production, multi-user, cloud (AWS RDS) | Medium |
| MySQL / MariaDB | Existing institutional MySQL stack; REDCap-adjacent deployments | Medium |
| ClickHouse | Large trials, time-series audit data | Medium |

---

### Option A: SQLite (Default, Recommended for Getting Started)

SQLite is file-based with zero configuration. Perfect for learning and
...
**Pros**: Zero setup, portable file, great for development
**Cons**: Single writer at a time, not ideal for multi-server

---

### Option B: DuckDB (Analytics-Optimized)

DuckDB is columnar and optimized for analytical queries. Excellent for
studies with heavy reporting needs.
...
**Pros**: Fast analytics, direct Parquet export, columnar compression
**Cons**: Less mature than SQLite for OLTP workloads
...
### Option C: PostgreSQL (Enterprise/Production)

PostgreSQL is the recommended backend for production deployments,
especially with multiple concurrent users or cloud hosting (AWS RDS).
...
**Pros**: Multi-user, ACID transactions, AWS RDS compatible, mature
**Cons**: Requires server installation/configuration
...
# Create database
createdb demonstration_trial

# Create user (optional)
psql -c "CREATE USER zzedc_user WITH PASSWORD 'your_password';"
psql -c "GRANT ALL PRIVILEGES ON DATABASE demonstration_trial TO zzedc_user;"
```
...
services:
  postgres:
    image: postgres:15
    environment:
      POSTGRES_DB: demonstration_trial
      POSTGRES_USER: zzedc_user
      POSTGRES_PASSWORD: ${ZZEDC_PG_PASSWORD}
    ports:
      - "5432:5432"
    volumes:
...
### Option D: ClickHouse (High-Volume Analytics)

ClickHouse is optimized for time-series data and high-volume audit trails.
Best for large trials with extensive logging requirements.
...
**Pros**: Blazing fast aggregations, columnar compression, time-series
**Cons**: No transactions, eventual consistency, requires server

**Installing ClickHouse (Docker)**:
```bash
...
docker exec -it clickhouse-server \
  clickhouse-client -q "CREATE DATABASE IF NOT EXISTS demonstration_trial"
```

**Installing ClickHouse R Package**:
```r
...
## Part 5: Create Database (Backend-Agnostic)

### Using the Setup Script

The setup script creates the complete database with all tables, data,
and indexes. It works with any backend you've configured.
...
1. **Creates database** using your configured backend
2. **Creates tables** with dialect-appropriate DDL:
...
```
Creating Demonstration Clinical Trial Database...
=========================================
...
Database location: ./data/demonstration_trial.db
Ready to launch ZZedc with demonstration trial data
=========================================
```
...
└── scripts/                          # R setup scripts
    ├── 01-setup_demonstration_trial.R          # Create database
    ├── 02-add_users.R                # Add users
    └── 03-verify_demonstration_trial.R         # Verify data
```
...
## Part 9: Using with ZZedc

### Launch ZZedc with Demonstration Trial
...
## Part 10: Testing Feature #1 (Encryption)

The demonstration trial is perfect for testing ZZedc's encryption features:
...
After setting up the demonstration trial:
...
- **Deployment / Encryption / Disaster Recovery**: See
  `vignette('technical-lead-guide')` (Chapters 5, 8, 9).
- **Custom Modules and REST API**:
  See `docs/development/custom-modules-and-api.md`.
- **Regulatory Compliance**: See
  `docs/compliance/gdpr-and-cfr-part11.md`.
