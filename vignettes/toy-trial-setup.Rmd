...
Before setting up the toy trial, you need to install ZZedc. If you have not
...
# 3. Install packages (validation first)
...
If you want to skip the detailed explanations and set up the toy trial
...
# Run the complete setup script (installs dependencies, packages, and toy trial)
...
2. Install zzedc.validation package
3. Install zzedc package
4. Create the toy trial database with 20 subjects
5. Add test users
6. Verify the setup
...
```{r quick_launch}
library(zzedc)
Sys.setenv(ZZEDC_DB_PATH = "vignettes/toy-trial/data/toy_trial.db")
launch_zzedc()
```
...
```bash
Rscript vignettes/toy-trial/scripts/01-setup_toy_trial.R
Rscript vignettes/toy-trial/scripts/02-add_users.R
Rscript vignettes/toy-trial/scripts/03-verify_toy_trial.R
```
