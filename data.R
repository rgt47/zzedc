      ...
      column(3,
        wellPanel(
          ...
          conditionalPanel(
            ...
            textInput("db_connection", "Database Connection:", 
          ...
          checkboxInput("show_summary", "Show Summary Statistics", TRUE),
          checkboxInput("show_missing", "Highlight Missing Values", TRUE)
        )
      ),
        ...
        tabsetPanel(
          ...
          tabPanel("Visualizations",
            ...
            fluidRow(
              column(6,
                wellPanel(
                  selectInput("viz_var_x", "X Variable:", choices = NULL),
                  selectInput("viz_var_y", "Y Variable:", choices = NULL),
              ...
              column(6,
                plotlyOutput("data_visualization")
              )
            )
          )
        )
    ...
    fluidRow(
      column(12,
        ...
        wellPanel(
          ...
          actionButton("validate_data", "Validate Data", class = "btn-warning"), 
          downloadButton("export_current_data", "Export Current View", class = "btn-success"),
          downloadButton("export_summary_report", "Export Summary Report", class = "btn-info")
        )
      )
    )
  )
...
current_data <- reactive({
  req(input$data_source)  # Ensure data_source is available
...
output$data_table <- DT::renderDataTable({
  data <- current_data()
  req(data, nrow(data) > 0)  # Ensure data exists and has rows

  max_rows <- req(input$max_rows)  # Ensure max_rows is available
