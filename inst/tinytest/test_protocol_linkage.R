...
# Test: init_protocol_linkage creates tables
local({
    setup_pl_test_env()
...
# Test: create_protocol_definition creates protocol
local({
    setup_pl_test_env()
...
# Test: create_protocol_definition validates required fields
local({
    result <- create_protocol_definition(
      protocol_number = "",
      protocol_title = "Test",
...
# Test: add_protocol_objective adds objective
local({
    setup_pl_test_env()
    init_protocol_linkage()
...
# Test: add_protocol_objective supports multiple objectives
local({
    setup_pl_test_env()
...
# Test: link_crf_to_protocol creates link
local({
    setup_pl_test_env()
    init_protocol_linkage()
...
# Test: link_crf_to_protocol with objective
local({
    setup_pl_test_env()
...
# Test: get_protocol_definitions retrieves protocols
local({
    setup_pl_test_env()
...
# Test: get_protocol_definitions filters inactive
local({
    setup_pl_test_env()
...
# Test: get_protocol_crf_links retrieves links
local({
    setup_pl_test_env()
    init_protocol_linkage()
...
# Test: get_protocol_linkage_statistics returns stats
local({
    setup_pl_test_env()
