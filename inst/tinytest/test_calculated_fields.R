...
# Test: init_calculated_fields creates tables
local({
    setup_cf_test()
...
# Test: load_standard_formulas loads formulas
local({
    setup_cf_test()
    on.exit(cleanup_cf_test())
...
# Test: get_standard_formulas retrieves formulas
local({
    setup_cf_test()
    on.exit(cleanup_cf_test())
...
# Test: get_standard_formulas filters by category
local({
    setup_cf_test()
    on.exit(cleanup_cf_test())
...
# Test: create_calculated_field creates field
local({
    setup_cf_test()
    on.exit(cleanup_cf_test())
...
# Test: create_calculated_field validates inputs
local({
    result <- create_calculated_field(
      calc_code = "",
      calc_name = "Test",
...
# Test: calculate_bmi calculates correctly
local({
...
# Test: calculate_bmi handles errors
local({
...
# Test: calculate_age calculates correctly
local({
...
# Test: calculate_age handles errors
local({
...
# Test: calculate_date_diff calculates correctly
local({
...
# Test: calculate_date_diff handles errors
local({
    result <- calculate_date_diff(NULL, "2025-01-01")
    expect_false(result$success)

})
# Test: calculate_bsa calculates correctly
local({
...
# Test: calculate_bsa handles errors
local({
    result <- calculate_bsa(NULL, 175)
    expect_false(result$success)

})
# Test: calculate_score_sum sums correctly
local({
...
# Test: calculate_score_sum handles NA
local({
...
# Test: calculate_score_sum handles empty
local({
    result <- calculate_score_sum(c())
    expect_false(result$success)

})
# Test: record_calculation records
local({
    setup_cf_test()
    on.exit(cleanup_cf_test())
...
# Test: get_calculated_fields retrieves fields
local({
    setup_cf_test()
    on.exit(cleanup_cf_test())
...
# Test: get_calculation_history retrieves history
local({
    setup_cf_test()
    on.exit(cleanup_cf_test())
...
# Test: get_calculated_fields_statistics returns stats
local({
    setup_cf_test()
    on.exit(cleanup_cf_test())
