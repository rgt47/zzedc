...
# Skip all tests if adverse event functions are not available
if (!(exists("init_adverse_events"))) exit_file("Adverse events functions not available")
...
# Test: init_adverse_events creates required tables
local({
...
# Test: get_ae_severity_grades returns all severity grades
local({
...
# Test: get_sae_criteria returns all SAE criteria
local({
...
# Test: get_causality_categories returns all categories
local({
...
# Test: get_ae_outcomes returns all outcome categories
local({
...
# Test: get_action_taken_categories returns all categories
local({
...
# Test: create_adverse_event creates AE successfully
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: create_adverse_event validates severity
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: create_adverse_event validates causality
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: create_adverse_event validates outcome
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: create_adverse_event creates hash chain
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: upgrade_to_sae upgrades AE to SAE
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: upgrade_to_sae requires at least one criterion
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: upgrade_to_sae prevents double upgrade
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: create_sae creates SAE directly
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: expedited deadline is 7 days for death/life-threatening
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: expedited deadline is 15 days for other SAEs
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: add_ae_followup creates followup record
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: add_ae_followup validates followup type
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: add_ae_followup maintains hash chain
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: resolve_adverse_event closes AE
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: resolve_adverse_event validates outcome
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: update_ae_causality updates causality with history
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: update_ae_causality validates causality
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: record_sae_notification records notification
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: record_sae_notification validates notification type
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: get_pending_sae_reports returns pending reports
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: get_adverse_event retrieves AE by ID
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: get_adverse_event retrieves AE by number
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: get_subject_adverse_events retrieves all subject AEs
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: get_ae_followups retrieves followup history
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: get_ae_statistics returns comprehensive stats
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: generate_ae_report creates TXT report
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: generate_ae_report creates JSON report
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: full AE lifecycle workflow
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: SAE expedited reporting workflow
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
