...
# Test: init_retention creates all required tables
local({
...
# Test: get_retention_bases returns valid bases
local({
...
# Test: get_retention_actions returns valid actions
local({
    action_types <- get_retention_actions()
    expect_equal(typeof(action_types), "character")
...
# Test: get_retention_statuses returns valid statuses
local({
...
# Test: get_retention_data_categories returns valid categories
local({
...
# Test: get_review_types returns valid types
local({
...
# Test: create_retention_policy creates policy
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: create_retention_policy validates policy_code required
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: create_retention_policy validates retention_period_days
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: create_retention_policy validates data_category
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: create_retention_policy validates retention_basis
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: create_retention_policy prevents duplicate codes
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: get_retention_policies retrieves policies
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: update_retention_policy updates policy
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: register_retention_record registers record
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: register_retention_record validates policy exists
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: register_retention_record prevents duplicates
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: register_retention_record creates audit trail
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: get_expired_records returns expired records
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: get_records_expiring_soon returns upcoming expirations
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: create_retention_review creates review
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: create_retention_review validates review_type
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: complete_retention_review completes review
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: delete_retention_record marks as deleted
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: delete_retention_record blocks for legal hold
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: anonymize_retention_record marks as anonymized
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: extend_retention extends period
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: extend_retention requires reason
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: apply_legal_hold places hold
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: apply_legal_hold requires reason
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: release_retention_hold releases hold
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: get_retention_record retrieves record
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: get_subject_retention_records retrieves subject records
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: get_retention_statistics returns stats
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: generate_retention_report creates TXT report
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: generate_retention_report creates JSON report
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: full retention lifecycle completes
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: legal hold blocks all destructive actions
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: hash chain integrity for retention actions
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
