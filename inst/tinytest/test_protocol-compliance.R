...
# Skip all tests if protocol compliance functions are not available
if (!(exists("init_protocol_compliance"))) exit_file("Protocol compliance functions not available")
...
# Test: init_protocol_compliance creates required tables
local({
...
# Test: create_protocol creates new protocol successfully
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: get_protocol retrieves protocol by ID
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: get_protocol retrieves protocol by code
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: duplicate protocol codes are rejected
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: add_protocol_visit adds visit successfully
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: get_protocol_visits retrieves all visits in order
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: visit windows are correctly defined
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: schedule_subject_visit schedules visit successfully
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: complete_subject_visit within window returns WITHIN_WINDOW
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: complete_subject_visit early creates deviation
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: complete_subject_visit late creates deviation
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: get_subject_visit_status returns visit status
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: get_deviation_types returns all valid types
local({
...
# Test: create_protocol_deviation creates deviation with hash chain
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: create_protocol_deviation validates deviation type
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: create_protocol_deviation validates severity
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: review_protocol_deviation updates deviation status
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: review_protocol_deviation validates status
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: get_protocol_deviations retrieves filtered deviations
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: add_eligibility_criterion creates inclusion criteria
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: add_eligibility_criterion creates exclusion criteria
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: add_eligibility_criterion validates criterion type
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: check_subject_eligibility passes for eligible subject
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: check_subject_eligibility fails for ineligible subject - exclusion met
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: check_subject_eligibility fails for ineligible subject - inclusion not met
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: evaluate_criterion handles different operators
local({
    expect_true(zzedc:::evaluate_criterion(25, "GT", "20"))
    expect_true(zzedc:::evaluate_criterion(20, "GE", "20"))
    expect_true(zzedc:::evaluate_criterion(15, "LT", "20"))
    expect_true(zzedc:::evaluate_criterion(20, "LE", "20"))
    expect_true(zzedc:::evaluate_criterion("A", "EQ", "A"))
    expect_true(zzedc:::evaluate_criterion("A", "NE", "B"))
    expect_true(zzedc:::evaluate_criterion("B", "IN", "A,B,C"))
    expect_true(zzedc:::evaluate_criterion("D", "NOT_IN", "A,B,C"))
    expect_true(zzedc:::evaluate_criterion("test123", "REGEX", "test[0-9]+"))
...
# Test: get_compliance_statistics returns comprehensive stats
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: generate_compliance_report creates TXT report
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: generate_compliance_report creates JSON report
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: full compliance workflow with multiple subjects
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: deviation hash chain maintains integrity
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
