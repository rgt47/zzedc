...
# Test: config.yml file exists and is readable
local({
...
# Test: config has required structure and fields
local({
    if (config_available()) local({
...
# Test: different environments have correct configurations
local({
    if (config_available()) local({
...
local({
    ...
    # Test database types
    expect_equal(typeof(cfg$database$path), "character")
    expect_equal(typeof(cfg$database$pool_size), "integer")

    # Test auth types
    expect_equal(typeof(cfg$auth$salt_env_var), "character")
    # Note: default_salt is intentionally null in config.yml, requires env var override
    # expect_equal(typeof(cfg$auth$default_salt), "character")
    expect_equal(typeof(cfg$auth$max_failed_attempts), "integer")

    # Test app types
    expect_equal(typeof(cfg$app$name), "character")
    expect_equal(typeof(cfg$app$version), "character")
    expect_equal(typeof(cfg$app$debug), "logical")

    })
})
# Test: configuration environment variable fallbacks work
local({
...
# Test: configuration handles missing files gracefully
local({
...
# Test: configuration supports database path variations
local({
    if (config_available()) local({
    config_file <- here::here("config.yml")
...
# Test: configuration auth settings are secure
local({
    if (config_available()) local({
    config_file <- here::here("config.yml")
...
# Test: configuration supports UI theme customization
local({
    if (config_available()) local({
    config_file <- here::here("config.yml")
    ...
    expect_equal(typeof(cfg$ui$theme), "character")
    expect_equal(typeof(cfg$ui$primary_color), "character")
...
# Test: configuration can be used for pool creation
local({
    cfg <- create_test_config()
...
# Test: configuration supports logging settings
local({
    if (config_available()) local({
    config_file <- here::here("config.yml")
