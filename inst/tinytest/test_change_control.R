...
# Test: init_change_control creates tables
local({
    setup_cc_test()
...
# Test: create_change_request creates request
local({
    setup_cc_test()
    on.exit(cleanup_cc_test())
...
# Test: create_change_request validates inputs
local({
    result <- create_change_request(
      request_title = "",
      request_type = "CRF_MODIFICATION",
...
# Test: submit_change_request submits
local({
    setup_cc_test()
    on.exit(cleanup_cc_test())
...
# Test: add_impact_assessment adds assessment
local({
    setup_cc_test()
    on.exit(cleanup_cc_test())
...
# Test: add_impact_assessment validates level
local({
    result <- add_impact_assessment(
      request_id = 1,
      impact_area = "Test",
...
# Test: get_impact_assessments retrieves assessments
local({
    setup_cc_test()
    on.exit(cleanup_cc_test())
...
# Test: add_change_approval adds approval
local({
    setup_cc_test()
    on.exit(cleanup_cc_test())
...
# Test: add_change_approval validates decision
local({
    result <- add_change_approval(
      request_id = 1,
      approval_role = "DM",
...
# Test: get_change_approvals retrieves approvals
local({
    setup_cc_test()
    on.exit(cleanup_cc_test())
...
# Test: add_implementation_step adds step
local({
    setup_cc_test()
    on.exit(cleanup_cc_test())
...
# Test: complete_implementation_step completes step
local({
    setup_cc_test()
    on.exit(cleanup_cc_test())
...
# Test: verify_implementation_step verifies step
local({
    setup_cc_test()
    on.exit(cleanup_cc_test())
...
# Test: verify_implementation_step validates result
local({
    result <- verify_implementation_step(
      implementation_id = 1,
      verified_by = "qa",
...
# Test: get_implementation_status returns status
local({
    setup_cc_test()
    on.exit(cleanup_cc_test())
...
# Test: update_change_request_status updates status
local({
    setup_cc_test()
    on.exit(cleanup_cc_test())
...
# Test: update_change_request_status validates status
local({
    result <- update_change_request_status(1, "INVALID_STATUS")
    expect_false(result$success)

})
# Test: get_change_requests retrieves requests
local({
    setup_cc_test()
    on.exit(cleanup_cc_test())
...
# Test: get_change_requests filters by status
local({
    setup_cc_test()
    on.exit(cleanup_cc_test())
...
# Test: get_change_request_details returns full details
local({
    setup_cc_test()
    on.exit(cleanup_cc_test())
...
# Test: get_change_request_details handles not found
local({
    setup_cc_test()
    on.exit(cleanup_cc_test())
...
# Test: get_change_control_statistics returns stats
local({
    setup_cc_test()
    on.exit(cleanup_cc_test())
