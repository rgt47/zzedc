...
# Test: list_available_instruments returns empty when no instruments
local({
    empty_dir <- tempdir()
...
# Test: list_available_instruments finds available instruments
local({
...
# Test: list_available_instruments returns metadata for known instruments
local({
    temp_dir <- create_test_instruments_dir()
...
# Test: load_instrument_template loads valid CSV
local({
    temp_dir <- create_test_instruments_dir()
...
# Test: load_instrument_template adds missing columns with defaults
local({
    temp_dir <- create_test_instruments_dir()
...
# Test: load_instrument_template errors on missing file
local({
    temp_dir <- create_test_instruments_dir()
...
# Test: load_instrument_template errors on missing required columns
local({
    temp_dir <- create_test_instruments_dir()
...
# Test: load_instrument_template validates field names
local({
    temp_dir <- create_test_instruments_dir()
...
# Test: load_instrument_template warns on unrecognized field types
local({
    temp_dir <- create_test_instruments_dir()
...
# Test: validate_instrument_csv validates file existence
local({
...
# Test: validate_instrument_csv validates CSV structure
local({
    temp_dir <- create_test_instruments_dir()
...
# Test: validate_instrument_csv detects duplicate field names
local({
    temp_dir <- create_test_instruments_dir()
...
# Test: validate_instrument_csv counts fields correctly
local({
    temp_dir <- create_test_instruments_dir()
...
# Test: get_instrument_field returns field by name
local({
    temp_dir <- create_test_instruments_dir()
    create_test_instrument_csv(file.path(temp_dir, "test.csv"), n_fields = 3)

    result <- zzedc:::get_instrument_field("test", "field_2", temp_dir)
...
# Test: get_instrument_field returns NULL if field not found
local({
    temp_dir <- create_test_instruments_dir()
    create_test_instrument_csv(file.path(temp_dir, "test.csv"), n_fields = 3)

    result <- zzedc:::get_instrument_field("test", "nonexistent", temp_dir)
...
# Test: import_instrument returns error without database connection
local({
    temp_dir <- create_test_instruments_dir()
    create_test_instrument_csv(file.path(temp_dir, "test.csv"), n_fields = 3)
...
# Test: import_instrument validates instrument exists
local({
    temp_dir <- create_test_instruments_dir()
...
# Test: list_available_instruments handles missing directory gracefully
local({
...
# Test: load_instrument_template handles special characters in descriptions
local({
    temp_dir <- create_test_instruments_dir()
...
# Test: validate_instrument_csv warns on non-CSV extension
local({
...
# Test: load_instrument_template recognizes all valid field types
local({
    temp_dir <- create_test_instruments_dir()
    ...
    expect_silent(result)  # Should not generate warnings
