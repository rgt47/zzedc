...
# Skip all tests if erasure functions are not available
if (!(exists("init_erasure"))) exit_file("Erasure functions not available")
...
# Test: init_erasure creates required tables
local({
...
# Test: init_erasure is idempotent
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: get_erasure_grounds returns valid grounds
local({
    grounds <- get_erasure_grounds()

    expect_equal(typeof(grounds), "list")
...
# Test: get_erasure_statuses returns valid statuses
local({
...
# Test: get_legal_hold_types returns valid types
local({
...
# Test: get_erasure_methods returns valid methods
local({
    methods <- get_erasure_methods()

    expect_equal(typeof(methods), "list")
...
# Test: get_erasure_exceptions returns valid exceptions
local({
    exceptions <- get_erasure_exceptions()

    expect_equal(typeof(exceptions), "list")
...
# Test: create_legal_hold creates hold successfully
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: create_legal_hold validates hold type
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: create_legal_hold requires minimum reason length
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: create_legal_hold with affected subjects and categories
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: release_legal_hold releases hold successfully
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: release_legal_hold requires minimum reason length
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: check_legal_hold detects matching hold
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: check_legal_hold returns no match when no hold applies
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: create_erasure_request creates request successfully
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: create_erasure_request validates erasure grounds
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: create_erasure_request detects legal hold
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: create_erasure_request sets 30-day due date
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: create_erasure_request creates hash chain
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: add_erasure_item adds item successfully
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: add_erasure_item validates erasure method
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: add_erasure_item detects category-based legal hold
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: review_erasure_item approves item successfully
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: review_erasure_item rejects item with reason
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: review_erasure_item blocks review of on-hold items
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: execute_erasure_item executes approved item
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: execute_erasure_item rejects unapproved item
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: add_erasure_third_party adds recipient successfully
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: notify_erasure_third_party records notification
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: confirm_erasure_third_party records confirmation
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: complete_erasure_request requires all items processed
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: complete_erasure_request completes successfully
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: reject_erasure_request rejects with reason
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: reject_erasure_request validates exception ground
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: get_erasure_request retrieves by ID
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: get_pending_erasure_requests excludes held by default
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: get_erasure_history retrieves history
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: get_erasure_statistics returns statistics
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: generate_erasure_report creates TXT report
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: generate_erasure_report creates JSON report
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: full erasure workflow completes successfully
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: legal hold blocks erasure workflow
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
