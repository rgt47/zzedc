...
# Test: init_crf_designer creates tables
local({
    setup_cd_test()
...
# Test: create_designer_form creates form
local({
    setup_cd_test()
    on.exit(cleanup_cd_test())
...
# Test: create_designer_form validates inputs
local({
    result <- create_designer_form(
      form_code = "",
      form_name = "Test",
...
# Test: add_form_section adds section
local({
    setup_cd_test()
    on.exit(cleanup_cd_test())
...
# Test: add_form_field adds field
local({
    setup_cd_test()
    on.exit(cleanup_cd_test())
...
# Test: add_form_field validates type
local({
    result <- add_form_field(
      design_id = 1,
      field_code = "TEST",
...
# Test: add_field_option adds option
local({
    setup_cd_test()
    on.exit(cleanup_cd_test())
...
# Test: update_field_position updates position
local({
    setup_cd_test()
    on.exit(cleanup_cd_test())
...
# Test: update_field_position requires updates
local({
    result <- update_field_position(field_id = 1)
    expect_false(result$success)

})
# Test: update_form_status updates status
local({
    setup_cd_test()
    on.exit(cleanup_cd_test())
...
# Test: update_form_status validates status
local({
    result <- update_form_status(
      design_id = 1,
      new_status = "INVALID",
...
# Test: lock_form_design locks form
local({
    setup_cd_test()
    on.exit(cleanup_cd_test())
...
# Test: get_designer_form returns full details
local({
    setup_cd_test()
    on.exit(cleanup_cd_test())
...
# Test: get_designer_form handles not found
local({
    setup_cd_test()
    on.exit(cleanup_cd_test())
...
# Test: get_designer_forms retrieves list
local({
    setup_cd_test()
    on.exit(cleanup_cd_test())
...
# Test: get_designer_forms filters by status
local({
    setup_cd_test()
    on.exit(cleanup_cd_test())
...
# Test: get_section_fields retrieves section fields
local({
    setup_cd_test()
    on.exit(cleanup_cd_test())
...
# Test: get_field_options retrieves options
local({
    setup_cd_test()
    on.exit(cleanup_cd_test())
...
# Test: delete_form_field deletes field
local({
    setup_cd_test()
    on.exit(cleanup_cd_test())
...
# Test: get_crf_designer_statistics returns stats
local({
    setup_cd_test()
    on.exit(cleanup_cd_test())
