...
# Test: init_portability creates all required tables
local({
...
# Test: init_portability is idempotent
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: get_portability_request_types returns valid types
local({
...
# Test: get_portability_legal_bases returns valid bases
local({
...
# Test: get_portability_export_formats returns valid formats
local({
    formats <- get_portability_export_formats()

    expect_equal(typeof(formats), "character")
...
# Test: get_portability_statuses returns valid statuses
local({
...
# Test: get_transfer_methods returns valid methods
local({
...
# Test: get_portability_exceptions returns valid exceptions
local({
    exceptions <- get_portability_exceptions()

    expect_equal(typeof(exceptions), "character")
    expect_true("RIGHTS_OF_OTHERS" %in% names(exceptions))
    expect_true("NOT_PROVIDED_BY_SUBJECT" %in% names(exceptions))

})
# Test: get_data_source_types returns valid sources
local({
    sources <- get_data_source_types()

    expect_equal(typeof(sources), "character")
...
# Test: create_portability_request creates RECEIVE request
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: create_portability_request requires target for TRANSMIT
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: create_portability_request creates TRANSMIT request with target
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: create_portability_request validates legal basis
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: create_portability_request sets 30-day due date
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: create_portability_request logs action
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: add_portability_dataset adds eligible dataset
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: add_portability_dataset marks derived data ineligible
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: add_portability_dataset validates data source
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: review_portability_dataset updates eligibility
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: review_portability_dataset requires reason for ineligible
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: generate_portability_export creates JSON export
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: generate_portability_export creates CSV export
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: generate_portability_export requires eligible datasets
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: record_export_download increments count
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: initiate_controller_transfer creates transfer
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: initiate_controller_transfer validates method
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: initiate_controller_transfer requires TRANSMIT type
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: complete_controller_transfer completes transfer
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: fail_controller_transfer records failure
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: complete_portability_request completes RECEIVE request
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: complete_portability_request requires export
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: complete_portability_request requires completed transfer for TRANSMIT
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: reject_portability_request rejects with reason
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: reject_portability_request requires minimum reason length
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: reject_portability_request validates exception ground
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: get_portability_request retrieves by ID
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: get_portability_request retrieves by number
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: get_portability_datasets filters by eligibility
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: get_pending_portability_requests returns pending
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: get_portability_statistics returns comprehensive stats
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: generate_portability_report creates TXT report
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: generate_portability_report creates JSON report
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: full RECEIVE portability workflow completes successfully
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: full TRANSMIT portability workflow completes successfully
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: rejection workflow works correctly
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
