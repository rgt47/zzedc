...
# Test: init_restriction creates all required tables
local({
...
# Test: init_restriction is idempotent
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: get_restriction_grounds returns valid grounds
local({
    grounds <- get_restriction_grounds()

    expect_equal(typeof(grounds), "character")
...
# Test: get_restriction_statuses returns valid statuses
local({
...
# Test: get_restriction_scopes returns valid scopes
local({
...
# Test: get_allowed_processing_during_restriction returns allowed types
local({
    allowed <- get_allowed_processing_during_restriction()

    expect_equal(typeof(allowed), "character")
...
# Test: create_restriction_request creates request successfully
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: create_restriction_request requires email
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: create_restriction_request validates grounds
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: create_restriction_request sets 30-day due date
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: create_restriction_request logs action
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: create_restriction_request detects legal hold
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: add_restriction_item adds item successfully
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: add_restriction_item validates scope
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: add_restriction_item detects legal hold on category
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: review_restriction_item approves item
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: review_restriction_item rejects with reason
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: review_restriction_item requires rejection reason
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: review_restriction_item blocks on_hold items
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: apply_restriction_item applies approved restriction
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: apply_restriction_item requires approval first
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: lift_restriction_item lifts active restriction
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: lift_restriction_item requires minimum reason length
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: check_restriction_status detects active restriction
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: check_restriction_status returns false when no restriction
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: log_processing_attempt logs blocked attempt
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: log_processing_attempt logs allowed override
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: add_restriction_third_party adds recipient
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: notify_restriction_third_party records notification
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: confirm_restriction_third_party requires notification first
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: confirm_restriction_third_party records confirmation
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: notify_lift_third_party records lift notification
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: approve_restriction_request approves request
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: activate_restriction_request activates with active items
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: activate_restriction_request requires approved status
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: lift_restriction_request lifts all active items
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: reject_restriction_request rejects with reason
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: reject_restriction_request requires minimum reason length
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: get_restriction_request retrieves by ID
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: get_restriction_request retrieves by number
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: get_restriction_items filters by status
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: get_pending_restriction_requests returns pending requests
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: get_active_restrictions returns active restrictions
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: get_restriction_statistics returns comprehensive stats
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: generate_restriction_report creates TXT report
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: generate_restriction_report creates JSON report
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: full restriction workflow completes successfully
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
...
# Test: restriction blocked by legal hold workflow
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db))
