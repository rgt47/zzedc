...
# Skip all tests if rectification functions are not available
if (!(exists("init_rectification"))) exit_file("Rectification functions not available")
...
# Test: init_rectification creates required tables
local({
...
# Test: init_rectification is idempotent
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: get_rectification_types returns valid types
local({
...
# Test: get_rectification_statuses returns valid statuses
local({
    statuses <- get_rectification_statuses()

    expect_equal(typeof(statuses), "list")
...
# Test: create_rectification_request creates request successfully
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: create_rectification_request sets 30-day due date
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: create_rectification_request validates request type
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: create_rectification_request with optional subject_id
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: create_rectification_request creates hash chain
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: add_rectification_item adds item successfully
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: add_rectification_item validates rectification type
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: add_rectification_item stores justification and evidence
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: review_rectification_item approves item successfully
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: review_rectification_item rejects with reason
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: review_rectification_item requires rejection reason
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: review_rectification_item validates decision
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: apply_rectification_item applies approved item
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: apply_rectification_item rejects unapproved item
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: add_third_party_recipient adds recipient successfully
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: add_third_party_recipient validates recipient type
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: add_third_party_recipient supports all valid types
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: send_third_party_notification records notification
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: send_third_party_notification validates method
local({
...
# Test: send_third_party_notification supports all methods
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: record_third_party_acknowledgment records acknowledgment
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: complete_rectification_request requires all items processed
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: complete_rectification_request requires all approved items applied
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: complete_rectification_request requires all notifications sent
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: complete_rectification_request completes successfully
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: reject_rectification_request rejects with reason
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: reject_rectification_request requires reason length
local({
    result <- reject_rectification_request(
      request_id = 1,
...
# Test: get_rectification_request retrieves by ID
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: get_rectification_request retrieves by number
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: get_rectification_items retrieves all items
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: get_rectification_items filters by status
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: get_rectification_history retrieves history
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: get_pending_rectification_requests retrieves pending
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: get_rectification_statistics returns statistics
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
    ...
    expect_true(nrow(stats$by_type) >= 1)
...
# Test: generate_rectification_report creates TXT report
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: generate_rectification_report creates JSON report
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: full rectification workflow completes successfully
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
...
# Test: hash chain integrity maintained across operations
local({
    test_db <- setup_test_db()
    on.exit(cleanup_test_db(test_db), add = TRUE)
