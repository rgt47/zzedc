...
# Supports multiple database backends via the adapter pattern

source("gsheets_integration.R")

#' Advanced data dictionary builder with form generation and validation
#'
#' Builds the data dictionary system from Google Sheets configuration.
#' Supports multiple database backends: SQLite, DuckDB, PostgreSQL, ClickHouse.
#'
#' @param dd_sheet_name Name of the Google Sheet containing data dictionary
#' @param config zzedc configuration list with db_adapter, or NULL to use global
#' @param db_path Path to database file (for file-based backends, deprecated -
#'   use config instead)
...
build_advanced_dd_system <- function(
  ...
  # Legacy support: if db_path provided but no config, create SQLite config
  if (is.null(config) && !is.null(db_path)) {
    warning("db_path parameter is deprecated. Use config with db_adapter instead.")
    config <- list(
      db_backend = "sqlite",
  ...
  if (is.null(config) || is.null(config$db_adapter)) {
    stop("No database configuration provided. Set up config with db_adapter.")
  }
  ...
  build_comprehensive_dd_tables(dd_data, config)
...
# ============================================================================
# Multi-Backend DDL Generation Helpers
# ============================================================================

#' Build column definitions for form data table
#'
#' Generates dialect-appropriate column definitions for a form's data table.
#'
#' @param form_def Data frame with form field definitions
#' @param dialect Dialect object from adapter
#' @param backend Backend name (sqlite, duckdb, postgresql, clickhouse)
#'
#' @return Character vector of column definition strings
#' @keywords internal
build_form_table_columns <- function(form_def, dialect, backend) {
  # Get auto-increment syntax for this backend
  auto_inc <- dialect$auto_increment %||% "INTEGER PRIMARY KEY"
  timestamp_now <- dialect$timestamp_now %||% "CURRENT_TIMESTAMP"

  # Base columns - adjust for backend
  if (backend == "clickhouse") {
    # ClickHouse: no auto-increment, use UInt64 for ID
    col_definitions <- c(
      "record_id UInt64",
      "subject_id String",
      "site_id Nullable(UInt32)",
      "visit_code Nullable(String)",
      "form_status String DEFAULT 'incomplete'",
      paste0("entry_date DateTime DEFAULT ", timestamp_now),
      "entry_user Nullable(UInt32)",
      "modified_date Nullable(DateTime)",
      "modified_user Nullable(UInt32)"
    )
  ...
  for (i in seq_len(nrow(form_def))) {
    field_name <- form_def$field[i]
    field_type <- form_def$type[i]

    sql_type <- map_field_type(field_type, backend)
...
#' Map EDC field type to SQL type for backend
#'
#' @param field_type EDC field type code (N, D, L, C)
#' @param backend Database backend name
#'
#' @return SQL type string
#' @keywords internal
map_field_type <- function(field_type, backend) {
  if (backend == "clickhouse") {
    switch(field_type,
      "N" = "Nullable(Float64)",
      "D" = "Nullable(Date)",
...
#'
#' Generates backend-appropriate CREATE TABLE statement.
#'
#' @param table_name Table name
#' @param col_definitions Character vector of column definitions
#' @param backend Database backend name
#' @param order_by Column(s) for ORDER BY (ClickHouse only)
#'
#' @return SQL CREATE TABLE statement
#' @keywords internal
build_create_table_sql <- function(table_name, col_definitions, backend,
                                    order_by = NULL) {
  if (backend == "clickhouse") {
    order_clause <- if (!is.null(order_by)) order_by else "tuple()"
    sprintf(
      "CREATE TABLE IF NOT EXISTS %s (\n  %s\n) ENGINE = MergeTree() ORDER BY (%s)",
      table_name,
      paste(col_definitions, collapse = ",\n  "),
      order_clause
    )
  } else {
    sprintf(
...
# ============================================================================
# Main Table Building Functions
# ============================================================================

#' Build comprehensive data dictionary tables including metadata
#'
#' Creates data dictionary tables using the configured database adapter.
#' Generates dialect-appropriate DDL for the active backend.
#'
#' @param dd_data List containing all data dictionary data
#' @param config zzedc configuration with db_adapter
build_comprehensive_dd_tables <- function(dd_data, config) {
  adapter <- config$db_adapter
  backend <- config$db_backend %||% "sqlite"
  dialect <- adapter$dialect()

  message("Building comprehensive data dictionary tables (backend: ", backend, ")")

  # Connect to database
con <- adapter$connect()
  on.exit(adapter$disconnect(con))
  ...
  tryCatch({
    ...
    for (form_name in names(form_definitions)) {
      ...
      # Build CREATE TABLE statement with dialect-appropriate types
      col_definitions <- build_form_table_columns(form_def, dialect, backend)

      # Build CREATE TABLE statement
      create_sql <- build_create_table_sql(
        table_name, col_definitions, backend,
        order_by = "record_id"  # For ClickHouse
      )
      dbExecute(con, create_sql)

      # Create indexes for performance (skip for ClickHouse - uses ORDER BY)
      if (backend != "clickhouse") {
        dbExecute(con, paste0(
          "CREATE INDEX IF NOT EXISTS idx_", table_name, "_subject ON ",
          table_name, "(subject_id)"
        ))
        dbExecute(con, paste0(
          "CREATE INDEX IF NOT EXISTS idx_", table_name, "_visit ON ",
          table_name, "(visit_code)"
        ))
      }
...
#'
#' Validates that the data dictionary tables exist and contain expected data.
#'
#' @param config zzedc configuration with db_adapter, or NULL to use global
#' @param db_path Deprecated. Use config instead.
verify_dd_system <- function(config = NULL, db_path = NULL) {
  ...
  # Legacy support
  if (is.null(config) && !is.null(db_path)) {
    warning("db_path parameter is deprecated. Use config with db_adapter.")
    config <- list(
      db_backend = "sqlite",
      sqlite = list(path = db_path)
    )
