...
test_that("Data pipeline runs successfully", {
  ...
  expect_true(summary(model).squared > 0)
})
...
test_that("Data validation passes", {
  ...
  actual_species <- unique(penguins)
  expect_true(all(actual_species %in% expected_species))
  
  # Check measurement ranges
  clean_penguins <- penguins[!is.na(penguins), ]
  expect_true(all(clean_penguins > 0))
  expect_true(all(clean_penguins < 100))  # Reasonable upper bound
  
  clean_mass <- penguins[!is.na(penguins), ]
  expect_true(all(clean_mass > 0))
  expect_true(all(clean_mass < 10000))  # Reasonable upper bound
})