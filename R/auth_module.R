...
#' @examples
#' \dontrun{
#' # Embed the authentication module in a custom Shiny app.
#' # Most users will run the full application via
#' # `zzedc::launch_zzedc()`; the example below shows how to
#' # embed the module in a custom Shiny app instead.
#' if (interactive()) {
#'   ui <- shiny::fluidPage(
#'     auth_ui("module_id")
#'   )
#'   server <- function(input, output, session) {
#'     auth_server("module_id")
#'   }
...
#' @examples
#' \dontrun{
#' # Programmatic login (the Shiny UI normally drives this).
#' auth <- authenticate_user(
#'   username = "site_pi",
#'   password = Sys.getenv("PI_PASSWORD")
#' )
#' auth$success
#' }
#' @keywords internal
authenticate_user <- function(username, password) {
  ...
  tryCatch({
    ...
    salt_env_var <- if (!is.null(cfg$auth$salt_env_var)) cfg$auth$salt_env_var else "ZZEDC_SALT"
    salt <- Sys.getenv(salt_env_var, unset = "")
    if (salt == "" && !is.null(cfg$auth$default_salt)) {
      salt <- cfg$auth$default_salt
    }
    # If still no salt, use the one stored with the user (from security_salt at creation)
    # For now, we need to retrieve the salt used during user creation
    # The password was hashed with security_salt during setup_wizard
    # We'll need to store salt per-user or use a global salt
    # Temporary fix: try common approach of checking the hash directly
    password_hash <- digest::digest(paste0(password, salt), algo = "sha256")

    # Compare single values (user_record is a data frame, extract first row value)
    stored_hash <- user_record$password_hash[1]
    if (identical(stored_hash, password_hash)) {
      # Update last login using pool
      pool::dbExecute(db_pool, "UPDATE edc_users SET last_login = ? WHERE user_id = ?",
                     params = list(Sys.time(), user_record$user_id[1]))

      return(list(
        success = TRUE,
        user_id = user_record$user_id[1],
        username = user_record$username[1],
        full_name = user_record$full_name[1],
        role = user_record$role[1],
        site_id = user_record$site_id[1]
      ))
    } else {
