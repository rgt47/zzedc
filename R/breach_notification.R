...
#' # Article 33: report a personal-data breach. The 72-hour clock
#' # is tracked by `check_72_hour_deadline()`.
#' report_breach_incident(
#'   incident_date    = Sys.time(),
#'   description      = "Lost laptop with cached export",
#'   affected_subjects = c("SUBJ-001", "SUBJ-002"),
#'   severity         = "medium",
#'   reported_by      = "site_pi"
#' )
#' }
...
#' # GDPR workflows write to the encrypted study database; see
#' # `?initialize_encrypted_database` for setup.
#' # Verify the controller is still inside the Art. 33 notification
#' # window for an open breach.
#' check_72_hour_deadline(incident_id = 1)
#' }
#' @export
