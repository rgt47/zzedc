...
#' )
#'
#' result <- validate_form_with_metadata(
...
#' }
#' }
validate_form_with_metadata <- function(form_data, field_metadata) {
  errors <- list()
  warnings <- list()
  cleaned_data <- list()
...
#' @examples
#' \dontrun{
#' # Internal helper used by `validate_form_with_metadata()` and the
#' # data-entry module. Numeric field with a plausibility range:
#' validate_field_value(
#'   field_name = "baseline_weight_kg",
#'   value      = 32,
#'   type       = "numeric",
#'   rules      = list(min = 30, max = 200)
#' )
#'
#' # Email field:
#' validate_field_value(
#'   field_name = "subject_email",
#'   value      = "subject@example.org",
#'   type       = "email",
#'   rules      = list()
#' )
#' }
...
setup_form_validation <- function(session, form_fields, field_metadata, error_container_id = "form_errors") {
  observe({
    ...
    # Validate form
    validation <- validate_form_with_metadata(form_data, field_metadata)
