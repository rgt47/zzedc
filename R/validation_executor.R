...
#' @param field_name Optional field name for profiling (extracted from validator
...
execute_validator <- function(validator, value, form_values = list(),
  ...
  # Get field name for profiling
  prof_field <- field_name %||% attr(validator, "field_name") %||% "unknown"

  with_profiling("validation", paste0("validate:", prof_field), {
    tryCatch(
      {
        ...
        if (isTRUE(result)) {
          ...
          list(valid = !isFALSE(result),
               error_message = "Validation returned unexpected type")
        }
      },
      error = function(e) {
  ...
  }, metadata = list(field = prof_field))
}
...
validate_form_fields <- function(form_values, validation_rules) {
  ...
  # Profile the entire batch validation
  with_profiling("validation", "batch_validate", {
    errors <- list()
    all_results <- list()
    is_valid <- TRUE
    ...
    for (field_name in names(validation_rules)) {
      ...
      tryCatch(
        {
          # Profile rule compilation separately
          validator <- with_profiling("validation", "compile_rule", {
            compile_validation_rule(rule, field_name)
          }, metadata = list(field = field_name))
        ...
        error = function(e) {
          all_results[[field_name]] <<- list(
            valid = FALSE,
            error_message = sprintf("Rule error: %s", e$message)
          )
  ...
  }, metadata = list(field_count = length(validation_rules)))
}
