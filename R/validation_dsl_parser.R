...
#
# Parses validation rule strings into Abstract Syntax Trees (AST) for code
...
# Token types for DSL lexer (internal constant, not exported)
TOKEN_TYPES <- list(
  EOF = "EOF",
  NUMBER = "NUMBER",
  STRING = "STRING",
...
tokenize_dsl_rule <- function(rule) {
  ...
  while (i <= nchar(rule)) {
    ...
    if (substr(rule, i, i) %in% c("'", '"')) {
      ...
      tokens[[length(tokens) + 1L]] <- Token(TOKEN_TYPES$STRING, str_value, line, column)
    ...
    if (grepl("^[0-9]", substr(rule, i, i))) {
      ...
      tokens[[length(tokens) + 1L]] <- Token(TOKEN_TYPES$NUMBER, as.numeric(num_str), line, column)
    ...
    if (grepl("^[a-zA-Z_]", substr(rule, i, i))) {
      ...
      if (!is.na(keyword_type)) {
        tokens[[length(tokens) + 1L]] <- Token(keyword_type, tolower(ident), line, column)
      } else {
        tokens[[length(tokens) + 1L]] <- Token(TOKEN_TYPES$IDENTIFIER, ident, line, column)
      }
    ...
    if (startsWith(rest, "..")) {
      tokens[[length(tokens) + 1L]] <- Token(TOKEN_TYPES$RANGE, "..", line, column)
      i <- i + 2
      column <- column + 2
    } else if (startsWith(rest, "==")) {
      tokens[[length(tokens) + 1L]] <- Token(TOKEN_TYPES$OPERATOR, "==", line, column)
      i <- i + 2
      column <- column + 2
    } else if (startsWith(rest, "!=")) {
      tokens[[length(tokens) + 1L]] <- Token(TOKEN_TYPES$OPERATOR, "!=", line, column)
      ...
      tokens[[length(tokens) + 1L]] <- Token(TOKEN_TYPES$LPAREN, "(", line, column)
      i <- i + 1
      column <- column + 1
    } else if (substr(rest, 1, 1) == ")") {
      tokens[[length(tokens) + 1L]] <- Token(TOKEN_TYPES$RPAREN, ")", line, column)
      i <- i + 1
      column <- column + 1
    } else if (substr(rest, 1, 1) == "{") {
      tokens[[length(tokens) + 1L]] <- Token(TOKEN_TYPES$LBRACE, "{", line, column)
      i <- i + 1
      column <- column + 1
    } else if (substr(rest, 1, 1) == "}") {
      tokens[[length(tokens) + 1L]] <- Token(TOKEN_TYPES$RBRACE, "}", line, column)
      i <- i + 1
      column <- column + 1
    } else if (substr(rest, 1, 1) == ",") {
      tokens[[length(tokens) + 1L]] <- Token(TOKEN_TYPES$COMMA, ",", line, column)
      i <- i + 1
      column <- column + 1
    } else {
  ...
  tokens[[length(tokens) + 1L]] <- Token(TOKEN_TYPES$EOF, "", line, column)
  tokens
}
...
# Parser class for DSL validation rules (internal)
# @noRd
Parser <- R6::R6Class(
  "Parser",
  public = list(
...
#'
#' @keywords internal
detect_validation_context_from_ast <- function(ast) {
