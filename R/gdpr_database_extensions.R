...
#' @examples
#' \dontrun{
#' # GDPR workflows write to the encrypted study database; see
#' # `?initialize_encrypted_database` for setup.
#' # Idempotent: ensures the GDPR data-subject tables exist in the
#' # study database. Called by `init()` for new projects.
#' add_gdpr_tables(db_path = "data/study.db")
#' }
#' @keywords internal
add_gdpr_tables <- function(db_connection) {
  ...
  cat("Creating data retention schedule table...\n")
  dbExecute(db_connection, "
  ...
  cat("Inserting default retention schedules...\n")
...
#' # GDPR workflows write to the encrypted study database; see
#' # `?initialize_encrypted_database` for setup.
#' # Quarterly GDPR compliance summary, suitable for the DPO.
#' rep <- generate_gdpr_compliance_report(
#'   quarter = "2026Q1",
...
#' # GDPR workflows write to the encrypted study database; see
#' # `?initialize_encrypted_database` for setup.
#' # Compliance score across the GDPR data-subject Articles for a
#' # study during a reporting window.
