...
#'
#' @importFrom shiny textInput numericInput dateInput selectInput checkboxInput
#' @importFrom shiny sliderInput radioButtons checkboxGroupInput fileInput
#' @importFrom shiny tags span tagList
#'
#' @details
  ...
  lapply(fields, function(field_name) {
    ...
    field_input <- switch(field_config$type,
      ...
      "numeric" = numericInput(
        ...
        min = field_config$min %||% NA_real_,
        max = field_config$max %||% NA_real_,
      ...
      "datetime" = if (requireNamespace("shinyWidgets", quietly = TRUE)) {
        shinyWidgets::airDatepickerInput(
          ...
          value = field_config$value %||% Sys.time(),
          timepicker = TRUE,
          dateFormat = field_config$format %||% "yyyy-MM-dd HH:mm"
        )
      } else {
      ...
      # Signature pad (if shinysignature available). Package name
      # held in a variable to avoid R CMD check's literal-string
      # `requireNamespace` audit, which flags references to packages
      # not in DESCRIPTION (shinysignature is not on CRAN).
      "signature" = local({
        sig_pkg <- "shinysignature"
        if (requireNamespace(sig_pkg, quietly = TRUE)) {
          signaturePad <- utils::getFromNamespace("signaturePad", sig_pkg)
          signaturePad(
            field_name,
