...
#' @examples
#' \dontrun{
#' # Embed the user and role management module.
...
#' if (interactive()) {
#'   ui <- shiny::fluidPage(
#'     user_management_ui("module_id")
#'   )
#'   server <- function(input, output, session) {
#'     user_management_server("module_id")
#'   }
...
#' @examples
#' \dontrun{
#' # Add a coordinator account before the first study visit.
#' save_user_to_db(
#'   username  = "new_coordinator",
#'   full_name = "J. Coordinator",
#'   email     = "jcoord@example.org",
#'   role      = "data_entry",
#'   password  = Sys.getenv("NEW_USER_PASSWORD")
#' )
#' }
#' @keywords internal
save_user_to_db <- function(db_pool, user_id, username, full_name, email, role, password, active, mode) {
  ...
  tryCatch({
    if (mode == "add") {
      # Hash the plaintext password using the admin-UI salt and
      # delegate to the canonical `db_insert_user()` helper so
      # the Shiny path, the gsheets setup, and the REDCap
      # importer all agree on the `edc_users` schema and INSERT.
      salt <- Sys.getenv("ZZEDC_SALT")
      if (salt == "") salt <- "default_salt"
      password_hash <- digest::digest(paste0(password, salt),
                                       algo = "sha256")
