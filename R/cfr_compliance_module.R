...
#' @examples
#' \dontrun{
#' # Embed the 21 CFR Part 11 compliance dashboard.
...
#' if (interactive()) {
#'   ui <- shiny::fluidPage(
#'     cfr_compliance_ui("module_id")
#'   )
#'   server <- function(input, output, session) {
#'     cfr_compliance_server("module_id")
#'   }
        ...
        display_data <- compliance_data$signatures %>%
          ...
          mutate(
            ...
            signature_status = case_when(
              signature_status == "valid" ~ "[OK] Valid",
              signature_status == "invalid" ~ "[X] Invalid",
