...
#' # Place a legal hold to block erasure during litigation or
#' # regulatory inquiry. Removes only via `release_legal_hold()`.
#' create_legal_hold(
#'   subject_id  = "SUBJ-001",
#'   reason      = "Pending FDA inspection 2026-Q3",
#'   placed_by   = "general_counsel",
#'   expiry_date = Sys.Date() + 365
#' )
#' }
#' @export
...
#' # GDPR workflows write to the encrypted study database; see
#' # `?initialize_encrypted_database` for setup.
#' # Right to erasure (Art. 17). The function records the request
#' # and the legal basis; actual erasure is staged via
#' # `execute_erasure_item()` after controller review.
#' create_erasure_request(
#'   subject_id   = "SUBJ-001",
#'   reason       = "withdrawal_of_consent",
#'   requestor    = "subject@example.org"
#' )
#' }
#' @export
