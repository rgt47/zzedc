...
#' @examples
#' \dontrun{
#' # Embed the GDPR data-subject portal.
...
#' if (interactive()) {
#'   ui <- shiny::fluidPage(
#'     privacy_ui("module_id")
#'   )
#'   server <- function(input, output, session) {
#'     privacy_server("module_id")
#'   }
      ...
      tryCatch({
        ...
        if (cfr_enabled() && exists("log_audit_event")) {
          log_audit_event(
            ...
            record_id = user_id,
            operation = paste("GDPR", request_type, "request submitted"),
            user_id = user_id
          )
        }
      ...
      tryCatch({
        ...
        if (cfr_enabled() && exists("log_audit_event")) {
          log_audit_event(
            ...
            record_id = user_id,
            operation = paste("GDPR consent", status, "for", consent_type),
            user_id = user_id
          )
        }
