...
#'   condition_expression = "dob < consent_date",
#'   error_message        = "Date of birth must precede consent date",
#'   created_by           = "biostat_lead",
#'   severity             = "ERROR",
#'   target_form          = "BASELINE"
#' )
#' rule$success
#' }
#' @export
...
#' @examples
#' \dontrun{
#' # Group baseline-visit consistency rules into a named set so they
#' # can be attached to the BASELINE form and re-used across studies.
#' set <- create_validation_rule_set(
#'   set_code        = "BASELINE_CHECKS",
#'   set_name        = "Baseline visit consistency checks",
#'   created_by      = "biostat_lead",
#'   set_description = "Cross-field rules required before locking baseline"
#' )
#' set$set_id
#' }
#' @export
...
#' @examples
#' \dontrun{
#' # Attach a rule to a rule set, controlling firing order so that
#' # cheap range checks run before expensive cross-form lookups.
#' add_rule_to_set(set_id = 3, rule_id = 17, execution_order = 10)
#' }
#' @export
